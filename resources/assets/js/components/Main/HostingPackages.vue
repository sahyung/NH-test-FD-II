        ...
        <div class="col-md-12  main-hostingpackages__packages">
          <ul class="packages-list">
            <li
              ...
              v-for="(hostingPackage, idx) in list.hostingPackages"
              :key="'package' + idx"
            >
                ...
                <div class="package-item__info">
                  <p class="package__description">
                    <del class="package__price-before"
                      ...
                      {{ hostingPackage.priceBeforeDiscount | decimalFormat }}
                    </del>
                    <ins class="package__price-after"
                      >{{ hostingPackage.currency }}
                      <span
                        class="package__price-after-discount"
                        v-html="discountedPrice(hostingPackage)"
                      >
                      </span>
                      /
                      {{ hostingPackage.subscriptionFrequency || 'bln' }}
                    </ins>
                  </p>
                </div>
                <div class="package-item__user">
                  <strong>
                    {{ hostingPackage.userCount | decimalFormat }}
                  </strong>
                  Pengguna Terdaftar
                </div>
                <div class="package-item__benefits">
                  <ul class="package__benefits benefit-list">
                    <li class="benefit-list__item">
                      {{ hostingPackage.resourcePower }}X resource power
                    </li>
                    <li
                      class="benefit-list__item"
                      v-for="(benefit, idx) in hostingPackage.benefits"
                      :key="'benefit' + idx"
                    >
                      <span class="benefit-name" v-html="benefit"></span>
                    </li>
                  </ul>
                  <button
                    type="button"
  ...
  methods: {
    ...
    discountedPrice(hostingPackage) {
      ...
      let priceAfterDiscountHtml = String(
        ...
        .split('.')
        .map((item, idx) => {
          return idx == 0 ? `<span class="pop-out">${item}</span>` : item;
        })
        .join('.');
      return priceAfterDiscountHtml;
    },
    getHostingPackages() {
      axios.get('api/hosting-packages').then((resp) => {
        this.list.hostingPackages = resp.data;
      });
    },
  },
};
